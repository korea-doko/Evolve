...
public class InputManager : MonoBehaviour, IManager
{
    ...
    void ButtonPressing()
    {
        m_currentDir = GetDir(m_buttonDownPos, Input.mousePosition);
        EventManager.GetInst().PressingDir(m_currentDir);      
    }
    void ButtonUp()
    {
        m_buttonDownPos = Vector3.zero;

        EventManager.GetInst().UpDir(m_currentDir); 
    }
